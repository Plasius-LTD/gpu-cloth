...
### Cloth Package Scope Clarification (Supersedes Fluid Package Scope Notes)
- Prior decision reference: `Fluid Package Scope Clarification`.
- Active rule: this repository is `@plasius/gpu-cloth`, not `@plasius/gpu-fluid`.
- Active rule: package-specific architecture in this repository must center cloth continuity, stable physics snapshot handoff, representation bands, and worker/performance integration for cloth preparation.
...
### Cloth Analytics and Debug Policy (Supersedes Fluid Analytics and Debug Policy)
- Prior decision reference: `Fluid Analytics and Debug Policy`.
- `@plasius/gpu-cloth` must not invent package-local analytics delivery.
- If cloth diagnostics need export beyond local hooks, route them through `@plasius/analytics`.
- If cloth diagnostics need runtime debug surfaces, prefer integration with `@plasius/gpu-debug` rather than bespoke package-local debug clients.

### Cloth Identity Clarification (Supersedes Earlier Package Identity Clarification)
- Prior decision reference: `Package Identity Clarification (Supersedes Package-Identity Interpretation Only)`.
- Active rule: package-identity, legal, and contributor-facing text in this repository must refer to `@plasius/gpu-cloth`.
- Clarification: earlier `@plasius/gpu-fluid` wording was superseded and is not the active package identity.

### Cloth Continuity Policy
- Near, mid, far, and horizon cloth representations must preserve shared silhouette, pinned-anchor identity, and broad-motion continuity across range transitions.
- Reduced distant fidelity is acceptable; cloth disappearance, detached anchors, or uncorrelated range-band motion is not.
